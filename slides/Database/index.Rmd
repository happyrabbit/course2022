...
- Intermediate: subquery, temporary table, join, window function
...
--
.pull-right[
- Advanced: data types/format, string and date, pivoting data
...
- Select the first 10 sales records with `Amount > 100` and only show the first 10 records
...
Example: Find the email of those customers who had an order with `Amount > 100` 

```sql
SELECT cus.Email, sale.Amount
FROM dim_customer AS cus
INNER JOIN fact_sales AS sale ON cus.PK_Customer_ID = sale.FK_Customer_ID
...
```sql
SELECT *, DENSE_RANK(Amount) OVER (PARTITION BY FK_Product_ID)
